<?php
 class dataConnection { 
	...
	public function getUserInfoWithEmail($email){
		$sql = sprintf("
			SELECT		u.userid, u.firstname, u.lastname, u.email, u.resetpassword, u.company, u.position, u.photofilename, u.createdate
			FROM		users u
			WHERE		u.email = '%s'
		",
			$this->esc($email)
	...
	public function tryPassReset($email, $temppassword){
		...
		if(trim($email)!= ""){
			...
			if(mysqli_affected_rows($this->_Link) == 1) $is_updated = true;
		}
		return $is_updated;
        }
	...
	public function getUserInfo($userid=0){
		...
		$sql = sprintf("
			SELECT		u.userid, u.firstname, u.lastname, u.email, u.createddate, u.photofilename,u.position, u.company, u.startdate,
	...
	public function adminUserList(){
		...
		$result = mysqli_query($this->_Link, $sql);
		if(!$result) $this->callError("SQL Error: " . $sql);
        }
...
// Add notes to mashlead_notes table
	...
	function callError($details='') {
		...
		if(!$result) $this->callError("Result error: " . $string);
		$queryresult = array();
		while ($row = mysqli_fetch_object($result)) {
			$queryresult[] = $row;
		}
